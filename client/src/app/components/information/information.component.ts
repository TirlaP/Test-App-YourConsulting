...
export class InformationComponent implements OnInit {
  ...
  constructor(private _modal: NgbModal, private _spinner: NgxSpinnerService) {
  ...
  addEdit = (id_information?: number): void => {
    const modalRef = this._modal.open(InformationModalComponent, {
      size: 'lg',
      keyboard: false,
      backdrop: 'static',
    });
  ...
  delete = (information: any): void => {
    ...
    modalRef.closed.subscribe(() => {
      axios
        .delete(`/api/information/${information.id}`)
        .then(() => {
